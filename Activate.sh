...
function point2()
{
  ...
  if test -z "$AS"; 
    then
		  idevicepair pair &>>/dev/nul& echo 'YOUR DEVICE HAS NOT BEEN DETECTED, IT WILL ATTEMPT TO PAIR WITH THE PC';
		  echo 'IF YOUR DEVICE IS CONNECTED, PRESS TRUST IN CASE PROMPTED';sleep 3;point1
    else
		 rm ~/.ssh/known_hosts&&iproxy 22 44 &>>/dev/nul&
		  echo 'DEVICE SUCCESSFULLY DETECTED!';
		  sleep 3; clear;
		  echo 'TRYING TO DISPLAY iDEVICE DATA';
		  sleep 2; echo ''; echo '';
		  echo 'acativation Status:  '$AS'';
...
echo "VERIFYING INSTALLATION OF DEPENDENCIES";  sleep 3; clear;
Check=$(find /usr/bin -iname "LibimobiledeviceEXE");
if test -z "$Check"; 
  then
    echo 'DOWNLOADING NECESSARY DEPENDENCIES'; sleep 2; echo '==============================================';
    cd /usr/bin/ && git clone https://github.com/Kangkarungs/LibimobiledeviceEXE; 
    cd /usr/bin/LibimobiledeviceEXE/ && mv ./* ../;
    echo '=============================================='; echo '';
    echo 'COMPLETED!'; sleep 3; point1;
  else
    point1;
fi
function Inicio() {	  
...
#If you have a Hacktivation server, generate your activation_record.plist at this point. 
#If it does, omit the lines that generate it.
...
#DEACTIVATING THE BASEBAND AFFECTS THE BATTERY LIFE, IF YOU DO NOT WANT THIS TO HAPPEN REMOVE THE LINE BasebandOFF
#AFTER DELETING THAT LINE LOAD COMMCENTER DAES AND ACTIVATE AIRPLANE MODE.
#IF YOU DO THIS YOU WILL NEED A SIMBCON PIN TO MAKE YOUR DEVICE UNTETHERED
